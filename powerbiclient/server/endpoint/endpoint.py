...
logger = logging.getLogger('powerbi.endpoint')
...
class Endpoint(object):
    ...
    def _make_common_headers(auth_token, content_type):
        return {
            'Content-Type': "{0}; charset=utf-8".format(content_type or 'application/json'),
            'Authorization': "Bearer {0}".format(auth_token)
        }

    @staticmethod
    def _safe_to_log(server_response):
        '''Checks if the server_response content is not JSON (eg binary image or zip)
        ...
        if server_response.headers.get('Content-Type', None) not in ALLOWED_CONTENT_TYPES:
            return '[truncated response contents]'
        else:
    ...
    def _make_request(self, method, url, content=None, request_object=None,
        ...
        if server_response.encoding:
            logger.debug(u'server response from {0}: {1}'.format(
    ...
    def _check_status(self, server_response):
        ...
        if server_response.status_code not in SUCCESS_CODES:
            raise ServerResponseError.from_response(server_response)
    ...
    def put_request(self, url, json_request, content_type='application/json'):
        return self._make_request(self.parent_srv.session.put, url,
                                  content=json_request,
                                  auth_token=self.parent_srv.auth_token,
                                  content_type=content_type)

    def post_request(self, url, json_request, content_type='application/json'):
    ...
    def _decorator(func):
        ...
        def wrapper(self, *args, **kwargs):
            ...
            if server_version < minimum_supported:
                error = "this endpoint is not available in API version {0}, requires {1}".format(
